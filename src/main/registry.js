...
function makeRegistry(baseDir) {
  ...
  function scan() {
    ...
    for (const entry of fs.readdirSync(versionsDir, { withFileTypes: true })) {
      ...
      const hasJar = fs.existsSync(path.join(dir, `${entry.name}.jar`));
      if (!hasStart && !hasJar) continue;
      if (!cleaned[entry.name]) {
        cleaned[entry.name] = {
          path: dir,
          icon: fs.existsSync(path.join(dir, "icon.png")) ? path.join(dir, "icon.png") : null,
          jvm: null,
          source: hasJar ? "microsoft" : "zip",
        };
      }
    }
  ...
  return { versionsDir, registryFile, load, save, scan, setDefaultJvm, upsert, remove, rename };
}
